...
bash 'generate keystore password' do
  ...
  EOH
  not_if do
    File.exist?("#{node['cassandra']['dse']['cassandra_ssl_dir']}/#{node['cassandra']['dse']['password_file']}")
  end
end
...
bash 'generate cassandra cert' do
  ...
  EOH
  not_if do
    File.exist?("#{node['cassandra']['dse']['cassandra_ssl_dir']}/#{node['hostname']}.keystore")
  end
end
...
bash 'export public key' do
  ...
  EOH
  not_if do
    File.exist?("#{node['cassandra']['dse']['cassandra_ssl_dir']}/#{node['hostname']}.cer")
  end
end
